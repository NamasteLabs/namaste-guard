...
export function generateHtmlReport(fileSummaries: string, fileDetails: string, totalFilesScanned: number, secretsLength: number, context: ExtensionContext): string {
    ...
    const logoPath = path.join(context.extensionPath, 'src', 'assets', 'img', 'namaste-guard-logo512.png');
    ...
    <head>
        ...
        <title>NamasteGuard Secrets Report</title>
        <style>
            ${cssContent}
        </style>
    </head>
    <body>
        <header>
            <img src="data:image/png;base64,${logoBase64}" alt="NamasteGuard Logo">
            <h1>NamasteGuard Secrets Report</h1>
        </header>
        <main>
        ...
        <footer>
            <p>&copy; <span id="current-year"></span> NamasteGuard. All rights reserved.</p>
            <script>
