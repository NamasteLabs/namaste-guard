...
function promptForSecretScan(context: vscode.ExtensionContext): void {
    vscode.window
        ...
        .then((selection) => {
            if (selection === 'Yes') {
                vscode.commands.executeCommand('namaste-guard.startScan', context);
            }
        });
}
...
/**
 * Ensures the `.namaste-guard` folder is added to `.gitignore` to prevent Git from tracking scan reports.
 * Prompts the user once before modifying the `.gitignore` files for all root folders and appends `.namaste-guard` if it’s not already listed.
 */
export async function addToGitIgnore(): Promise<void> {
    ...
    // Track which folders need `.namaste-guard` added
    ...
    for (const folder of workspaceFolders) {
        ...
        if (fs.existsSync(gitFolderPath) && fs.statSync(gitFolderPath).isDirectory()) {
            ...
            // Add to list if `.namaste-guard` is not already in `.gitignore`
            if (!gitignoreContent.includes('.namaste-guard')) {
    ...
    const response = await vscode.window.showInformationMessage(
        'The .namaste-guard folder stores scan reports. Add it to .gitignore to avoid Git tracking?',
        'Yes',
        'No'
    );
    ...
    if (response === 'Yes') {
        ...
        if (updatedFolders.length > 0) {
            vscode.window.showInformationMessage(
                `.namaste-guard folder added to .gitignore in ${updatedFolders.join(', ')}.`
            );
        } else {
            vscode.window.showInformationMessage('Failed to add .namaste-guard folder to .gitignore.');
        }
    }
}

/**
 * Checks the `namaste-guard.addToGitIgnore` setting and calls `addToGitIgnore` if enabled.
 */
export function checkAndAddToGitIgnore(): void {
    ...
    if (gitIgnoreBoolean) {
        addToGitIgnore(); // Add .namaste-guard to .gitignore if setting is enabled
    }
}
...
export function setupGitIgnoreListeners(context: vscode.ExtensionContext): void {
    ...
    const configChangeListener = vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration('namaste-guard.addToGitIgnore')) {
            checkAndAddToGitIgnore();
        }
    });
